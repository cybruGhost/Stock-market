...
import (
	...
	"github.com/cybruGhostStock-market"
	"github.com/nsf/termbox-go"
)

// File name in user's home directory where we store the settings.
const defaultProfile = `.stockmarketrc`

const help = `stockmarket v1.0.0 -- Copyright (c) 2013-2023 by Michael Dvorkin and contributors. All Rights Reserved.
...
// -----------------------------------------------------------------------------
func mainLoop(screen *stockmarket.Screen, profile *stockmarket.Profile) {
	var lineEditor *stockmarket.LineEditor
	var columnEditor *stockmarket.ColumnEditor
	...
	market := stockmarket.NewMarket()
	quotes := stockmarket.NewQuotes(market, profile)
	screen.Draw(market)
	screen.Draw(quotes)
			...
			case termbox.EventKey:
				if lineEditor == nil && columnEditor == nil && !showingHelp {
					if event.Key == termbox.KeyEsc || event.Ch == 'q' || event.Ch == 'Q' {
						...
						lineEditor = stockmarket.NewLineEditor(screen, quotes)
						...
						columnEditor = stockmarket.NewColumnEditor(screen, quotes)
...
func main() {
	...
	profile, err := stockmarket.NewProfile(*profileName)
	if err != nil {
	...
	screen := stockmarket.NewScreen(profile)
	defer screen.Close()
