...
// Profile manages stockmarket program settings as defined by user (ex. list of
// stock tickers). The settings are serialized using JSON and saved in
// the ~/.stockmarketrc file.
type Profile struct {
...
// Creates the profile and attempts to load the settings from ~/.stockmarketrc file.
...
// Save serializes settings using JSON and saves them in ~/.stockmarketrc file.
